import type { AssistRequestInput } from "./schema";
...
export function buildPrompt(request: AssistRequestInput) {
	const screenshotSummary = request.screenshots
		.map((screenshot, index) => {
			const activeLabel =
				screenshot.screenshotId === request.activeScreenshotId ? " (active)" : "";
